...
public class OpenIE {
	...
	public void extract(String path) {
        ...
        for(File f : files) {
        	...
        	saveTriples(filename.substring(lastSlash+1), triples);
        }
	}
	
	private void saveTriples(String filename, HashSet<String> triples) {
		try
        {
            FileWriter fw = new FileWriter(filename+".txt"); // 절대주소 경로 가능
